...
public class NotificationController {
    ...
    public ResponseEntity<ApiResponse<List<Notification>>> getMethodName(HttpServletRequest request) {
        ...
        List<Notification> notification = notificationRepository.findByUserIdAndDueDaysLessThanEqual(userId, 30);
    ...
    @GetMapping("/update-notifications-from-bills")
    public ResponseEntity<ApiResponse<String>> updateNotificationsFromBills(HttpServletRequest request) {
        UUID userId = UUID.fromString((String) request.getAttribute("userId"));

        notificationService.updateNotificationsFromBills(userId);

        ApiResponse<String> entity = new ApiResponse<>(true, "Notification Updated Successfully", null);
        return ResponseEntity.status(200).body(entity);
    }
